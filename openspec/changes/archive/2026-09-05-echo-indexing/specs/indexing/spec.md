...
The system SHALL embed each chunk through the configured LLM provider (`provider.embeddings`) and store vectors in `embeddings` with model and dimension metadata.
...
- **WHEN** embedding a batch fails (provider unreachable, rate limit, model error)
- **THEN** the system records the error in `index_state.error`, leaves the affected note's `structural_status` as `failed`, does not delete already-stored embeddings for other notes, and allows retry on the next run
